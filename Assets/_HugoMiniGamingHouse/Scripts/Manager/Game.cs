...
public class Game : Manager
{
    ...
    public int coins
    {
        ...
        {
            _coins = value;
            targetText.text = value + "/" + ((AppHugo.selectedLevel) * 1000);
            if (value >= (AppHugo.selectedLevel) * 1000)
            {
                ShowGameOver(true);
            }
        }
    ...
    public void ShowGameOver(bool isWin)
    {
        ...
        if (isWin)
        {
            ...
            gameData.AddMoney(1000);
            Time.timeScale = 0;
            if ((AppHugo.selectedLevel + 1) > AppHugo.saveLevel && AppHugo.selectedLevel <= 28)
                AppHugo.saveLevel = (AppHugo.selectedLevel + 1);
            coins = 0;
        }
        else
        {
